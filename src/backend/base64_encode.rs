...
fn set_content_type(content_type: &str ) -> Result<Value, Box<dyn Error>> {
    ...
    let mut parsed_contentType_json: Value = serde_json::from_str(content_json).unwrap();    

    match content_type {
        "PNG" => {
            parsed_contentType_json["id"] = Value::String("http://publications.europa.eu/resource/authority/file-type/PNG".to_string());
            parsed_contentType_json["prefLabel"]["en"][0] = Value::String("PNG".to_string());
        }
        "JPG" => {
            parsed_contentType_json["id"] = Value::String("http://publications.europa.eu/resource/authority/file-type/JPG".to_string());
            parsed_contentType_json["prefLabel"]["en"][0] = Value::String("JPG".to_string());
        }
        "SVG" => {
            parsed_contentType_json["id"] = Value::String("http://publications.europa.eu/resource/authority/file-type/SVG".to_string());
            parsed_contentType_json["prefLabel"]["en"][0] = Value::String("SVG".to_string());
        }
         _ => {
            // Handle other cases
            return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput,"This file Type is not exporteswent wrong!")));
        }
    }
    Ok(parsed_contentType_json)
    }
...
fn set_content_enconding_type(content_encoding_type: &str ) -> Result<Value, Box<dyn Error>> {
  ...
  match content_encoding_type {
      "base64" => {
          parsed_content_encoding_type_json["id"] = Value::String("http://data.europa.eu/snb/encoding/6146cde7dd".to_string());
          parsed_content_encoding_type_json["prefLabel"]["en"][0] = Value::String("base64".to_string());
      }
       _ => {
          // Handle other cases
          return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput,"This content encoding Type is not supported!")));
      }
  }
...
fn set_language(language: &str ) -> Result<Value, Box<dyn Error>> {
  ...
  match language {
      "ENG" => {
          parsed_language_json["id"] = Value::String("http://publications.europa.eu/resource/authority/language/ENG".to_string());
          parsed_language_json["prefLabel"]["en"][0] = Value::String("English".to_string());
      }
      "NLD"=> {
          parsed_language_json["id"] = Value::String("http://publications.europa.eu/resource/authority/language/NLD".to_string());
          parsed_language_json["prefLabel"]["en"][0] = Value::String("dutch".to_string());
      }
       _ => {
          // Handle other cases
          return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput,"This language Type is not supported!")));
      }
  }
...
pub fn image_to_individual_display(image_value: Value) -> Value {
        ...
        "displayDetail": [
          {
            ...
            "image": {
              ...
              "contentType": {
                "id": "http://publications.europa.eu/resource/authority/file-type/JPG",
                "type": "Concept",
                "inScheme": {
                  "id": "http://publications.europa.eu/resource/authority/file-type",
                  "type": "ConceptScheme"
    ...
    let encoded_string = match encode_image_from_url("https://raw.githubusercontent.com/hamrt/credential-converter/refs/heads/image/test/edubadges.png") {
        Ok(encoded_string) => {
    ...
    let encoded_content_type = match set_content_type("PNG") {
  ...
  if let Some(image_encoding) = parsed_json["displayDetail"][0]["image"]["contentEncoding"].as_object() {
